...
class SqfliteHelper {
  ...
  Future<Database> openMyDatabase() async {
    return await openDatabase(
      ...
      onCreate: (db, version) {
        return db.execute(
          'CREATE TABLE $todoList($id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, $title TEXT, $description TEXT, $status INTEGER)',
        );
  ...
  Future<void> updateTitle(
    ...
    db.update(
      todoList,
      {title: titleData, description: descriptionData},
      where: 'id = ?',
      whereArgs: [id],
    );
  }
  ...
  Future<void> deleteAllSelectedTasks(List<int> ids) async {
    final db = await openMyDatabase();
    // delete method takes two arguments: the name of the table and the where clause.
    // we are using the list of ids for the where clause to delete all tasks with the given ids.
    final idPlaceholders = ids.map((_) => '?').join(',');
    db.delete(todoList, where: 'id IN ($idPlaceholders)', whereArgs: ids);
  }
}
